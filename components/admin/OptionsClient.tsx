...
const emptyGroup = { name_fr: "", name_ar: "", name_en: "", required: "false", min_select: "0", max_select: "1" };
    ...
    <div className="flex flex-col gap-4">
      ...
      {showAdd && (
        <div className="rounded-2xl bg-white p-5" style={{ border: "2px solid #1877F2" }}>
          ...
          <GroupFormFields form={tmplForm} setForm={setTmplForm} onSave={handleCreateTemplate} onCancel={() => setShowAdd(false)} />
        </div>
      )}
          ...
          <div className="flex items-center gap-3 px-5 py-4" style={{ borderBottom: "1px solid #E4E6EB", background: "#FAFBFC" }}>
              ...
              <>
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs mt-0.5" style={{ color: "#65676B" }}>
                    {tmpl.required ? "Requis" : "Optionnel"} · {tmpl.minSelect}–{tmpl.maxSelect} choix · {tmpl.options.length} option{tmpl.options.length !== 1 ? "s" : ""}
                  </p>
                </div>
                {/* Apply button */}
                <button
                ...
                <button
                  onClick={() => { setEditId(tmpl.id); setTmplForm({ name_fr: tmpl.name.fr, name_ar: tmpl.name.ar, name_en: tmpl.name.en, required: String(tmpl.required), min_select: String(tmpl.minSelect), max_select: String(tmpl.maxSelect) }); }}
            ...
            {editId === tmpl.id && (
              <div className="flex-1">
                ...
                <GroupFormFields form={tmplForm} setForm={setTmplForm} onSave={handleUpdateTemplate} onCancel={() => setEditId(null)} />
              </div>
            )}
          </div>
                ...
                {tmpl.options.map(opt => (
                  <div key={opt.id}>
                    {editOptId === opt.id ? (
                      ...
                      <div className="flex items-center gap-3 rounded-xl px-3 py-2.5" style={{ background: "#F0F2F5" }}>
                        ...
                        <span className="text-xs" style={{ color: "#65676B" }}>FR · AR · EN</span>
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-3 gap-3">
        <label className="flex flex-col gap-1">
          ...
          <select value={form.required} onChange={e => setForm({ ...form, required: e.target.value })} className="rounded-xl px-3 py-2 text-sm outline-none" style={{ border: "1.5px solid #E4E6EB", color: "#1C1E21", background: "#FAFBFC" }}>
            <option value="false">Non</option>
            <option value="true">Oui</option>
          </select>
        </label>
        <LabelInput label="Min choix" value={form.min_select} onChange={e => setForm({ ...form, min_select: e.target.value })} type="number" />
        <LabelInput label="Max choix" value={form.max_select} onChange={e => setForm({ ...form, max_select: e.target.value })} type="number" />
      </div>
